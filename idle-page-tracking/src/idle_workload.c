 ...
 *
 * Creates a single file-backed mmap() split into three regions:
 ...
 *
 * Experiment phases:
 *   Phase 0 — create the mapping and prefault all regions
			   ...
			   size_t unused_size)
{
	...
	fprintf(f,
		...
		"  }\n"
		"}\n",
		(long)getpid(), page_size, start, total,
		start, hot_size,
...
int main(int argc, char **argv)
{
	...
	while ((c = getopt_long(argc, argv, "", opts, NULL)) != -1) {
		...
		case 'u': unused_size = parse_size(optarg); break;
		case 'r':
	 ...
	 *
	 * Build one page-aligned file-backed mapping, split it into the three
	 * experiment regions, and touch every page once so all target pages are
	 * resident before the idle observation window begins.
	 */
	...
	if (hot_size != orig_hot || stream_size != orig_stream ||
	    unused_size != orig_unused)
		fprintf(stderr,
			"note: region sizes rounded up to page size %zu\n",
			page_size);

	size_t total = hot_size + stream_size + unused_size;
	...
	fprintf(stderr,
		...
		(void *)unused, unused_size);

	fprintf(stderr, "prefaulting all regions...\n");
	...
	write_metadata(meta_path, page_size, base, total,
		       hot_size, stream_size, unused_size);
